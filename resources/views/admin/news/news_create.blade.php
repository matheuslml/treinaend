        ...
        <div class="card-body ">
          ...
          <div class="col-lg-12 align-items-center auth-bg px-2 p-lg-5">
            ...
            @include('flash::message')
              ...
              @endif
              <form class="auth-register-form mt-2" action="{{ route('noticias.store') }}" method="POST" enctype="multipart/form-data">
                @csrf()
                  ...
                  <div class="row">
                    ...
                    <div class="col-md-4 mb-1">
                      <label class="form-label" for="category_id">Categoria<tag data-bs-toggle="tooltip" title="Não se esqueça de cadastrar previamente a categoria"><i data-feather='info'></i></tag></label>
                    ...
                    <div class="col-md-4 mb-1">
                      <label class="form-label" for="meta_keywords">Palavra Chave<tag data-bs-toggle="tooltip" title="Palavra para uxiliar nas pesquisa"><i data-feather='info'></i></tag></label>
                    ...
                    <div class="col-md-12 mb-1">
                      <label class="form-label" for="select2-multiple">TAG'S<tag data-bs-toggle="tooltip" title="Não se esqueça de cadastrar as tag's previamente"><i data-feather='info'></i></tag></label>
                      <select class="select2 form-select" id="tags" name="tags[]" multiple>
                        <optgroup label="Selecione">
                          @foreach($tags as  $tag)
                            <option value="{{ $tag->id }}" {{ (in_array($tag->id, old('tag', [])) || isset($legislation) && $legislation->tags->contains($tag->id)) ? 'selected' : '' }} >{{ $tag->tag }}</option>
                          @endforeach
                        </optgroup>
                      </select>
                    </div>
