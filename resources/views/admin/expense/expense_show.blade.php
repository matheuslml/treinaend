        ...
        <div class="card-body ">
          ...
          <div class="col-lg-12 align-items-center auth-bg px-2 p-lg-5">
            ...
            @include('flash::message')
              ...
              @endif
              <form class="auth-register-form mt-2" action="{{ route('despesas.update', $expense->id) }}" method="POST" enctype="multipart/form-data">
                @csrf()
                @method('PUT')
                  <div class="content-header mb-2">
                      <h2 class="fw-bolder mb-75">Dados da Despesa</h2>
                  </div>
                  <div class="row">
                    <div class="col-md-9 mb-1">
                      <label class="form-label" for="title">Descrição</label>
                    ...
                    <div class="col-md-3 mb-1">
                      ...
                      <select class="form-select" id="status" name="status" required>
                        ...
                        <option value="DRAFT" {{ $expense->status == 'DRAFT' ? 'selected' : '' }} >Desenvolvendo</option>
                        <option value="PENDING" {{ $expense->status == 'PENDING' ? 'selected' : '' }} >Pendente</option>
                        <option value="PUBLISHED" {{ $expense->status == 'PUBLISHED' ? 'selected' : '' }} >Publicada</option>
                      </select>
                    </div>
                    ...
                    <div class="col-md-4 mb-1">
                      <label class="form-label" for="genre">Lista de Tipos</label>
                      <select class="form-select" id="type_expense_id" name="type_expense_id" >
                        <option value="" class="">Tipos</option>
                        @foreach($types as $type)
                          <option value="{{ $type->id }}" {{ $type->id == $expense->type_expense_id ? 'selected' : '' }}>{{ $type->title }}</option>
                        @endforeach
                      </select>
                    </div>
                    <div class="col-md-3 mb-1">
                      <label class="form-label" for="current_balance">Saldo Atual</label>
                      <input type="text" class="form-control current-balance" placeholder="1.250,25" id="current_balance" name="current_balance" 
                        value="{{ str_replace('.',',', $expense->current_balance) }}"
                      />
                    </div>
                    <div class="col-md-3 mb-1">
                      <label class="form-label" for="blocked_balance">Saldo Bloqueado</label>
                      <input type="text" class="form-control blocked-balance" placeholder="1.250,25" id="blocked_balance" name="blocked_balance" 
                        value="{{ str_replace('.',',', $expense->blocked_balance) }}"
                      />
                    </div>
                    <div class="col-md-3 mb-1">
                      <label class="form-label" for="used_balance">Saldo Utilizado</label>
                      <input type="text" class="form-control used-balance" placeholder="1.250,25" id="used_balance" name="used_balance" 
                        value="{{ str_replace('.',',', $expense->used_balance) }}"
                      />
                    </div>
                    <div class="col-md-3 mb-1">
                      <label class="form-label" for="available_balance">Saldo Disponível</label>
                      <input type="text" class="form-control available-balance" placeholder="1.250,25" id="available_balance" name="available_balance" 
                        value="{{ str_replace('.',',', $expense->available_balance) }}"
                      />
                    </div>
                    <div class="col-md-12 mb-1">
                      <label class="form-label" for="notes">Anotações</label>
                      <textarea type="text" name="notes" id="notes" class="form-control">{{ $expense->notes }}</textarea>
                    </div>
                  ...
                  <div class="row">
                    <div class="col-12">
                      ...
                      <form method="POST" name="form-delete" action="{{ route('despesas.destroy', $expense->id) }}">
                          @csrf()
                          @method('delete')
                          <button type="submit" class="btn btn-danger" style="position: relative; float: left;" 
                            onclick="return confirm('Tem certeza que deseja deletar a Despesa?');">Deletar
                          </button>
                      </form>
                    </div>
                  </div>
    ...
    <div class="col-12">
      <div class="card">
        <div class="card-header">
          <h4 class="card-title">Adicionar Arquivos desta Despesa</h4>
        </div>
        <div class="card-body">
          <p class="card-text">
          ...
          @csrf()  
            <input type="text" name="type" id="type" class="form-control" value="expense" hidden />
            <input type="text" name="id" id="id" class="form-control" value="{{ $expense->id }}" hidden />
