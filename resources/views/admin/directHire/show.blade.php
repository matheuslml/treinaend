        ...
        <div class="card-body ">
          ...
          <div class="col-lg-12 align-items-center auth-bg px-2 p-lg-5">
            ...
            @include('flash::message')
              ...
              @endif
              <form class="auth-register-form mt-2" action="{{ route('contratacoes_diretas.update', $direct_hire->id) }}" method="POST" >
                @csrf()
                @method('PUT')
                  <div class="content-header mb-2">
                      <h2 class="fw-bolder mb-75">Dados da Contratação Direta</h2>
                  </div>
                  <div class="row">
                    <div class="col-md-12 mb-1">
                      <label class="form-label" for="title">Nome</label>
                      <input type="text" name="title" id="title" class="form-control" value="{{ $direct_hire->title }}" />
                    </div>
                    <div class="col-md-3 mb-1">
                      <label class="form-label" for="status">Status</label>
                      <select class="form-select" id="status" name="status" required>
                        <option value="" class="">Selecione</option>
                        <option value="DRAFT" {{ $direct_hire->status == 'DRAFT' ? 'selected' : '' }} >Desenvolvendo</option>
                        <option value="PENDING" {{ $direct_hire->status == 'PENDING' ? 'selected' : '' }} >Pendente</option>
                        <option value="PUBLISHED" {{ $direct_hire->status == 'PUBLISHED' ? 'selected' : '' }} >Publicada</option>
                      </select>
                    </div>
                    <div class="col-md-3 mb-1">
                      ...
                      <select class="form-select" id="modality_id" name="modality_id" >
                        ...
                        @foreach($modalities as $modality)
                          <option value="{{ $modality->id }}" {{ $direct_hire->modality_id===$modality->id ? 'selected' : '' }} >{{ $modality->title }}</option>
                        @endforeach
                      </select>
                    </div>
                    <div class="col-md-3 mb-1">
                      ...
                      <select class="form-select" id="situation_id" name="situation_id" >
                        ...
                        @foreach($situations as $situation)
                          <option value="{{ $situation->id }}" {{ $direct_hire->situation_id===$situation->id ? 'selected' : '' }} >{{ $situation->title }}</option>
                        @endforeach
                      </select>
                    </div>
                    <div class="col-md-3 mb-1">
                      <label class="form-label" for="bidding">Contratação Direta</label>
                      <input type="text" name="bidding" id="bidding" class="form-control" value="{{ $direct_hire->bidding }}" />
                    </div>
                    <div class="col-md-4 mb-1">
                      <label class="form-label" for="value_min">Valor Mínimo</label>
                      <input type="text" class="form-control value_min"  value="{{ str_replace('.',',', $direct_hire->value_min) }}" id="value_min" name="value_min" />
                    </div>
                    <div class="col-md-4 mb-1">
                      <label class="form-label" for="value_max">Valor Estimativa</label>
                      <input type="text" class="form-control value_max"  value="{{ str_replace('.',',', $direct_hire->value_max) }}" id="value_max" name="value_max" />
                    </div>
                    <div class="col-md-4 mb-1">
                      <label class="form-label" for="local">Local</label>
                      <input type="text" name="local" id="local" class="form-control" value="{{ $direct_hire->local }}" />
                    </div>
                    <div class="col-md-12 mb-1">
                      <label class="form-label" for="content">Objeto</label>
                      <input type="text" name="content" id="content" class="form-control" value="{{ $direct_hire->content }}" />
                    </div>
                    <div class="col-md-4 mb-1">
                      <label class="form-label" for="process">Processo</label>
                      <input type="text" name="process" id="process" class="form-control" value="{{ $direct_hire->process }}" />
                    </div>

                    <div class="col-md-4 mb-1">
                      <label class="form-label" for="published_at">Data de Publicação do Edital</label>
                      <input type="date" name="published_at" id="published_at" class="form-control" value="{{ date('Y-m-d',strtotime($direct_hire->published_at)) }}" />
                    </div>
                    <div class="col-md-4 mb-1">
                      <label class="form-label" for="realized_at">Data da licitação</label>
                      <input type="date" name="realized_at" id="realized_at" class="form-control" value="{{ date('Y-m-d',strtotime($direct_hire->realized_at)) }}" />
                    </div>
                  ...
                  <div class="col-12 mt-1 pb-4">
                    ...
                    <form method="POST" name="form-delete" action="{{ route('contratacoes_diretas.destroy', $direct_hire->id) }}">
                        @csrf()
                        @method('delete')
  ...
  <div class="row">
    <div class="col-md-4 col-12">
      <div class="card">
        ...
        <div class="card-body">
          <form class="form form-horizontal" method="POST" action="{{ route('contratacao_direta_vencedores.store') }}">
            @csrf()
            <input type="text" name="direct_hire_id" value="{{ $direct_hire->id }}" hidden />
            <div class="row">
              <div class="col-12">
                <div class="mb-1 row">
    ...
    <div class="col-md-8 col-12">
      <div class="card">
        ...
        @if (count($direct_hire->winners) >= 1)
          <table class="dt-advanced-search-winner table">
            ...
            <tbody>
              @php $i = 0; @endphp
              @foreach($direct_hire->winners as $winner)
                @if($i == 0)
                  @php $i = 1; @endphp
                  <tr class="odd">
                @else
...
<!-- Vencedor ends -->
  ...
  <div class="row">
    <div class="col-12">
      <div class="card">
        <div class="card-header">
          <h4 class="card-title">Adicionar Arquivos desta Contratação Direta</h4>
        </div>
        <div class="card-body">
          <p class="card-text">
          ...
          @csrf()  
            <input type="text" name="type" id="type" class="form-control" value="direct_hire" hidden />
            <input type="text" name="id" id="id" class="form-control" value="{{ $direct_hire->id }}" hidden />
      ...
      <div class="card">
        ...
        @if (count($direct_hire->files) >= 1)
          <table class="dt-advanced-search table">
            ...
            <tbody>
              @php $i = 0; @endphp
              @foreach($direct_hire->files as $file)
                @if($i == 0)
                  @php $i = 1; @endphp
                  <tr class="odd">
                @else
  ...
  <div class="row">
    <div class="col-md-4 col-12">
      <div class="card">
        ...
        <div class="card-body">
          <form class="form form-horizontal" method="POST" action="{{ route('contratacao_direta_itens.store') }}">
            @csrf()
            <input type="text" name="direct_hire_id" value="{{ $direct_hire->id }}" hidden />
            <div class="row">
              <div class="col-12">
      ...
      <div class="card">
        ...
        @if (count($direct_hire->items) >= 1)
          <table class="dt-advanced-search-bond table">
            ...
            <tbody>
              @php $i = 0; @endphp
              @foreach($direct_hire->items as $item)
                @if($i == 0)
                  @php $i = 1; @endphp
                  <tr class="odd">
                @else
