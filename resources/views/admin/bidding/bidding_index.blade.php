    ...
    <div class="col-lg-3 col-sm-6">
      <div class="card">
        <div class="card-body d-flex align-items-center justify-content-between">
          <div>
            <h3 class="fw-bolder mb-75">{{ count($biddings) }}</h3>
    ...
    <div class="col-12">
      <div class="card">
        <div class="card-header border-bottom">
          <h4 class="card-title">Licitações - Busca Avançada</h4>
        </div>
        <!--Search Form -->
              ...
              @foreach($biddings as $bidding)
                @if(isset($bidding->title))
                  ...
                  @endif
                      ...
                      <td style="display: none;">{{ $bidding->title }}</td>
                      <td style="display: none;">{{ $bidding->modality->title }}</td>
                      <td style="display: none;">{{ $bidding->situation->title }}</td>
                      <td style="display: none;">{{ $bidding->status == 'PENDING' ? 'Pendente' : ($bidding->status == 'DRAFT' ? 'Editando' : 'Publicado') }}</td>
                      <td style="display: none;">{{isset($bidding->created_at) ? (($bidding->created_at)->format('d/m/Y H:m:s')) : ''}}</td>
                      <td style="display: none;">

                        <div class="btn-group">
                          <a href="{{ route('licitacoes.show', $bidding->id) }}" class="btn btn-info">
