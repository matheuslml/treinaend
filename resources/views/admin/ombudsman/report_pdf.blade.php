        ...
        <main class="o-main" style="clear:both;">
            <div class="row">
                <div class="document-master">
                    ...
                    <div class="info-data">
                        <div class="col col-5">
                            <p>Total de manifestações Marcadas: {{ count($ombudsman) }}</p>
                        </div>
                        <div class="col col-5" >
                            <p style="text-align: right;">Arraial do Cabo, {{date('d/m/Y - H:i')}} horas</p>
                        </div>
                        <div class="row col col-10 index-table" style="margin-top: 5%;">
                            <h4>Manifestações Cadastradas deste Relatório </h4>
                            <div class="col-10">   
                            @if (count($ombudsman) >= 1)
                                <table class="table">
                                <thead>
                                    <tr>
                                    <th>Título</th>
                                ...
                                <tbody>
                                    @foreach($ombudsman as $ombudsmen)
                                        <tr>
                                            <td>{{ $ombudsmen->title }}</td>
                                            <td>{{ $ombudsmen->name }}</td>
                                            <td>{{ $ombudsmen->type_request->title }}</td>
                                            <td>{{ $ombudsmen->type_access->title }}</td>
                                            <td>{{ $ombudsmen->email }}</td>
                                            <td>{{ $ombudsmen->content }}</td>
                                            <td>{{ date('d-m-Y', strtotime($ombudsmen->created_at)) }}</td>
                                        </tr>
                                    @endforeach
                                </tbody>
                                </table>
