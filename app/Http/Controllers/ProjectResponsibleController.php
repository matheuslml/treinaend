...
use App\Http\Requests\ProjectResponsibleRequest;
use App\Models\Unit;
...
class ProjectResponsibleController extends Controller{
    ...
    public function __construct(
        ...
        protected ProjectResponsibleCreateService $projectResponsibleCreateService,
        protected ProjectResponsibleUpdateService $projectResponsibleUpdateService,
    ){}
...
$courses_nav = Course::where('status', 'PUBLISHED')->get();
        ...
        $responsibles = ProjectResponsible::with('projects')->latest()->get();
        return view('admin.project.responsible_index', ['pageConfigs' => $pageConfigs], compact('responsibles', 'unit', 'copyright', 'courses_nav'));
...
public function store(
    ProjectResponsibleRequest $request
){
...
public function show($responsible_id)
{
    ...
    try{
        ...
        $unit = Unit::where('web', true)->first();
        $copyright = Copyright::where('status', 'PUBLISHED')->first();
        return view('admin.project.responsible_show', compact('responsible_selected', 'responsibles', 'unit', 'copyright', 'courses_nav'));
...
public function update(
    ProjectResponsibleRequest $request, $responsible_id
){
...
public function destroy($project_responsible)
{
    ...
    try{
        $for_delete = ProjectResponsible::find($project_responsible);
        $for_delete->delete();
        flash('Responsável deletado com sucesso!')->success();
        return redirect('/projeto_responsaveis');
    } catch (\Exception $exception) {
